...
3. Certificate generation:-
    A. cd ./certs
    B. openssl req -x509 -newkey rsa:4096 -keyout ./certs/moodle.key -out ./certs/moodle.crt -days 365 -nodes
    C. cd ../
4. Update DB details in entrypoint.sh file-
    A. MYSQL_USER, MYSQL_PASSWORD, MYSQL_DATABASE

5. docker build --no-cache -t my-moodle .
6. docker run -d -p 80:80 -p 443:443 --name moodle-container my-moodle
7. docker ps -a
8. docker exec -it moodle-container /bin/bash
...
### Update System and Install Apache, MariaDB, and PHP
sudo apt update
sudo apt upgrade -y
...
sudo mysql_secure_installation #Follow the prompts to set the root password and remove insecure default settings.
